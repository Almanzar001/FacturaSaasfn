-- Actualizar función de estadísticas del dashboard para incluir ingresos generales

-- Primero, actualizar la función básica de estadísticas
...
DECLARE
  ...
  total_invoice_revenue numeric := 0;
...
BEGIN
  ...
  BEGIN
    SELECT COALESCE(SUM(total), 0) INTO total_invoice_revenue
    FROM public.invoices
  ...
  -- Total revenue = invoice revenue + general income
  total_revenue := total_invoice_revenue + total_general_income;
  ...
  RETURN jsonb_build_object(
    ...
    'totalInvoiceRevenue', total_invoice_revenue,
...
DECLARE
  ...
  prev_invoice_revenue numeric := 0;
...
BEGIN
  ...
  -- Get invoice revenue (current month)
  BEGIN
    SELECT COALESCE(SUM(total), 0) INTO total_invoice_revenue
    FROM public.invoices
    WHERE organization_id = org_id 
  ...
  -- Previous month invoice revenue
  BEGIN
    SELECT COALESCE(SUM(total), 0) INTO prev_invoice_revenue
    FROM public.invoices
    WHERE organization_id = org_id 
    AND status = 'paid'
    AND created_at >= prev_period_start 
  ...
  prev_revenue := prev_invoice_revenue + prev_general_income;
...
SELECT 'Funciones de dashboard actualizadas para incluir ingresos generales.' as mensaje;